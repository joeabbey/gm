...
function buildTableContent(claudeData, codexData) {
  ...
  // Sort dates and create interlaced rows
  ...
  // Header
  lines.push('┌──────────────┬────────┬────────────────────┬───────────┬───────────┬───────────┬───────────┬───────────┐');
  lines.push('│ {bold}Date{/bold}         │ {bold}Source{/bold} │ {bold}Model(s){/bold}           │ {bold}Input{/bold}     │ {bold}Output{/bold}    │ {bold}CacheRead{/bold} │ {bold}CacheWrit{/bold} │ {bold}Cost{/bold}      │');
  lines.push('├──────────────┼────────┼────────────────────┼───────────┼───────────┼───────────┼───────────┼───────────┤');

  // Data rows
  rows.forEach(row => {
    const { date, source, data, color } = row;
    ...
    if (models.length > MODEL_W) {
      models = models.substring(0, MODEL_W - 3) + '...';
    }

    const datePart = date.padEnd(DATE_W);
    const sourcePart = source.padEnd(SOURCE_W);
    const modelsPart = models.padEnd(MODEL_W);
    ...
    const cwritePart = formatNumber(cacheWrite).padStart(CWRITE_W);
    const costPart = formatCost(cost).padStart(COST_W);

    const line = `│ ${datePart} │ {${color}}${sourcePart}{/${color}} │ {${color}}${modelsPart}{/${color}} │ ${inputPart} │ ${outputPart} │ ${creadPart} │ ${cwritePart} │ ${costPart} │`;
    lines.push(line);
  });
...
function runUsageWatch(options = {}) {
  ...
  function updateDisplay() {
    ...
    header.setContent(
      ...
      `Last Update: ${now}  │  Refreshing every ${interval}s  │  Press {bold}q{/bold} to quit, {bold}↑↓{/bold} to scroll`
    );
