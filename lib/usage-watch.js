...
function formatNumber(num) {
  if (num >= 1000000000) {
    return `{green-fg}{bold}${(num / 1000000000).toFixed(1)}B{/bold}{/green-fg}`;
  }
  if (num >= 1000000) {
    return `{bold}${(num / 1000000).toFixed(1)}M{/bold}`;
  }
  if (num >= 1000) {
    return `${(num / 1000).toFixed(1)}K`;
  }
  return `{gray-fg}${num.toString()}{/gray-fg}`;
}
...
function buildTableContent(claudeData, codexData) {
  ...
  sortedDates.forEach(date => {
    ...
    entries.forEach(entry => {
      ...
      if (source === 'Claude') {
        ...
        if (data.models) {
          modelBreakdowns = Object.entries(data.models).map(([modelName, modelData]) => {
            ...
            // Codex's inputTokens includes cached tokens, so subtract to get net new input
            const netInput = (modelData.inputTokens || 0) - (modelData.cachedInputTokens || 0);
      ...
      if (modelBreakdowns.length === 0) {
        const inputValue = source === 'Codex'
          ? (data.inputTokens || 0) - (data.cachedInputTokens || 0)  // Codex: subtract cached from input
          : (data.inputTokens || 0);  // Claude: input is already net
  ...
  // Header
  lines.push('┌──────────────┬────────┬────────────────────┬─────────────┬─────────────┬─────────────┬─────────────┬─────────────┬───────────┐');
  lines.push('│ {bold}Date{/bold}         │ {bold}Source{/bold} │ {bold}Model{/bold}              │ {bold}Input{/bold}       │ {bold}Output{/bold}      │ {bold}CacheRead{/bold}   │ {bold}CacheCreate{/bold} │ {bold}Total{/bold}       │ {bold}Cost{/bold}      │');
  lines.push('├──────────────┼────────┼────────────────────┼─────────────┼─────────────┼─────────────┼─────────────┼─────────────┼───────────┤');

  // Helper to calculate visible length (excluding tags)
  const getVisibleLength = (str) => str.replace(/\{[^}]+\}/g, '').length;

  // Helper to pad a string accounting for invisible tag characters
  const padWithTags = (str, width) => {
  ...
  rows.forEach(row => {
    ...
    const inputPart = padWithTags(formatNumber(input), INPUT_W);
    const outputPart = padWithTags(formatNumber(output), OUTPUT_W);
    const creadPart = padWithTags(formatNumber(cacheRead), CREAD_W);
    const ccreatePart = padWithTags(formatNumber(cacheWrite), CCREATE_W);
    const totalPart = padWithTags(formatNumber(total), TOTAL_W);
  ...
  if (claudeData && claudeData.totals) {
    ...
    const inputPart = padWithTags(formatNumber(t.inputTokens || 0), INPUT_W);
    const outputPart = padWithTags(formatNumber(t.outputTokens || 0), OUTPUT_W);
    const creadPart = padWithTags(formatNumber(t.cacheReadTokens || 0), CREAD_W);
    const ccreatePart = padWithTags(formatNumber(t.cacheCreationTokens || 0), CCREATE_W);
    const totalPart = padWithTags(formatNumber(total), TOTAL_W);
  ...
  if (codexData && codexData.totals) {
    ...
    // Codex's inputTokens includes cachedInputTokens, so we need to subtract for display
    const netInput = (t.inputTokens || 0) - (t.cachedInputTokens || 0);
    const total = netInput + (t.outputTokens || 0) + (t.cachedInputTokens || 0);
    const datePart = (claudeData && claudeData.totals ? '' : 'TOTAL').padEnd(DATE_W);
    const sourcePart = 'Codex'.padEnd(SOURCE_W);
    const modelsPart = ''.padEnd(MODEL_W);
    const inputPart = padWithTags(formatNumber(netInput), INPUT_W);
    const outputPart = padWithTags(formatNumber(t.outputTokens || 0), OUTPUT_W);
    const creadPart = padWithTags(formatNumber(t.cachedInputTokens || 0), CREAD_W);
    const ccreatePart = padWithTags(formatNumber(0), CCREATE_W);
    const totalPart = padWithTags(formatNumber(total), TOTAL_W);
